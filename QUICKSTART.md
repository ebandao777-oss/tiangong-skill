...
| 天工能帮我执行 Agent 吗？      | 不能。天工只设计+验证 Agent 的 prompt，不运行 Agent。运行任务需派发给对应 Sub Agent。详见 [能力摘要](SKILL.md#-能力摘要)                             |
| 天工能帮我找现成的 Agent 吗？  | 不能。天工是创作工具，不推荐/搜索已有工具。找现成的用 app-agent 或主 Agent 当前可用的外部检索工具。                                                  |
