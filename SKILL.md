...
从调研素材中提取 3-7 个心智模型，使用四重验证筛选（详见 [角色思维框架方法论](references/extraction-framework.md)）：

| 验证维度 | 检查问题 | 通过标准 | 未通过处理 |
|---------|---------|---------|-----------|
| 跨域复现 | 在 ≥3 个不同职业场景中是否出现相同思维路径？ | 不是单一场景的即兴发挥 | 降级为「情境观点」 |
| 生成力 | 能否用该模型预测人物对新职业任务的反应？ | 模型有预测能力，不是事后总结 | 降级为「事后总结」 |
| 排他性 | 换一位同岗位从业者套用此模型是否明显违和？ | 具备此人独特的思维烙印 | 标记为「行业共性」，移除 |
| 职业合规 | 是否符合该职业的强制性规范与伦理底线？ | 是 | 一票否决，禁止Skill化 |

四重全部通过 → 心智模型；仅 1-3 重 → 降级为决策启发式；0 重 → 丢弃。职业合规一票否决即无条件移除。
...
>
> 表达DNA提取完成后，按 extraction-framework §2 三级门槛体系补全：深度蒸馏 → 6 项全量统计 + 全场景锚点；标准模式 → 4 项统计（平均句长/术语频次/确定性风格/默认姿态）+ 核心场景锚点；精简模式 → 仅定义场景锚点变化方向，不做量化统计。详见 [角色思维框架方法论](references/extraction-framework.md) §2。

- **价值观与反模式**：价值排序 + 明确反对的行为/思维方式
- **内在张力**：价值观之间的冲突——深度的来源

> 职业实践张力提取（详见 [角色思维框架方法论](references/extraction-framework.md) §3）：所有矛盾统一归入「职业实践中的张力」，禁止拆分为「人物内心冲突」与「职业伦理冲突」。按三级优先级提取：
>
> 1. **职业本质性张力**（岗位核心价值间的冲突，如司法公正 vs 个体正义）→ 最高优先级，必须保留并显式标注职业根源
> 2. **职业-个人认知张力**（个体经验与行业规范的摩擦）→ 次优先级，标注触发场景与个体的调和方式
> 3. **跨场景行为差异**（不同职业场景下的表达/决策差）→ 基础优先级，作为动态锚点写入表达DNA
>
> 同层级矛盾按「职业场景出现频次 × 对决策的影响强度」加权排序。无法排序时保留矛盾本身，标注 `[未解职业张力]`。无依据推测标记为「待验证假设」，不得写入正式Skill。
...
2.5. **融合可信度测试**（详见 [角色思维框架方法论](references/extraction-framework.md) §4）：
...
| 测试项 | 操作方法 | 通过标准 | 失败处理 |
| :--- | :--- | :--- | :--- |
| 职业情境辨识度测试 | 新会话加载 Skill，输入模糊职业任务，问「这是谁在处理？」 | 3 次内准确识别 | 返回蒸馏 2 重写职业场景锚点 |
| 职业边界压力测试 | 输入超出该岗位职责范围的问题 | 触发职业边界机制，不越界编造 | 返回蒸馏 2 补全职业诚实边界 |
| 风格漂移测试 | 连续对话 N 轮职业任务后检查语言特征（深度蒸馏 20 轮 / 标准模式 10 轮 / 精简模式 5 轮） | 量化指标波动 ≤15% | 强化动态锚点约束 |
| 同行盲审（可选但推荐） | 邀请该职业从业者评估 Skill | 确认符合真实工作逻辑 | 根据反馈修正心智模型 |

全部 4 项清单勾选 + 前 3 项注入测试通过 = 融合可信度 PASS。任一不通过 → 回退蒸馏 2 修复对应模块。
