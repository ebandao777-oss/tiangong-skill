...
| 岗位型标准设计 | 非必需 | ≥5 篇行业权威材料 | 覆盖 ≥3 种职业情境 |
| 快速原型（两型通用） | ≥1 篇 | ≥2 篇 | — |

> 不论类型，推断素材均不得单独支撑任何心智模型。素材不足时逐级降级：深度蒸馏 → 标准模式 → 精简模式 → 逃生通道。禁止强行提取。
>
> ⚠️ **逃生通道**：当连精简模式门槛（≥1 一手 + ≥2 二手）也无法满足时，进入**推断辅助模式**——允许以推断素材为主力构建心智模型，但必须在产出物开头显式标注「⚠️ 低信度原型：素材不足，心智模型基于推断构建，建议后续补充一手素材后重新蒸馏」。
...
5.  **体系映射表**：本框架与天工技能核心流程的对应关系如下，供执行时参照。

| extraction-framework | 映射到 SKILL.md / 模板 | 关系说明 |
|---------------------|----------------------|---------|
| §0 素材质量标准 | 蒸馏 1 调研 + Gate 1 信息完备门禁 | framework 定义素材门槛，SKILL.md 执行调研和门禁判定 |
| §1 四重验证 / 岗位型适应 | 蒸馏 2 心智模型 + Gate 2 | framework 定义验证维度，SKILL.md 执行筛选和门禁 |
| §2 表达DNA + 三级门槛 | 蒸馏 2 表达DNA + persona-template §表达DNA + job-template §表达DNA | framework 定义指标体系和场景锚点，蒸馏 2 收集数据，模板落表 |
| §2 非职业场景基线 | persona-template §情绪触发条件 + job-template §表达DNA | framework 定义基线维度，模板中并入对应章节 |
| §3 职业实践张力 | 蒸馏 2 矛盾提取 + 两型模板 §内在张力/职业实践张力 | framework 定义优先级协议，蒸馏 2 按协议提取，模板落表 |
| §4 质量自检 | 蒸馏 4 融合可信度测试 + 两型模板 §质量自检 | framework 定义测试项，SKILL.md 执行，模板内嵌清单 |

> 本映射表用于确保 methodology 不在传递过程中逐级衰减。当 SKILL.md 或模板与 extraction-framework 出现差异时，以 framework 为准。
>
> 📋 **跨文件验证**：上表中对 SKILL.md 蒸馏编号、persona-template 章节名、Gate 机制名称的引用，需在实际跨文件审计中逐条验证是否存在。本表记录的是 design intent（设计意图），而非已验证的实现对应关系。
