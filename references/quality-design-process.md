...
> 以下为 Agent 内部参考——当上述异常发生时，Agent 应优先自动处理，仅在无法自愈时才暴露给用户。

| 用户可见异常 | Agent 应自动执行的恢复动作 | 自愈失败时的上报内容 |
|-------------|--------------------------|-------------------|
| 用户说"脚本报错了" | ① 读取完整 traceback；② 缺失依赖则 pip install 并重跑；③ 路径问题则切换到绝对路径重跑 | 「已尝试自动修复但 verify-skill.py 仍不可用，已改为手工逐条核对 YAML/CHECKPOINT/规则冲突，效果等同」 |
| 用户说"输出被截断了" | 自动识别截断点，将未输出的扩展模块在下一轮补全 | 不需要上报——截断处理对用户透明 |
| 用户说"搜不到资料" | ① 自动切换关键词/搜索引擎重搜；② 放宽条件后重搜；③ 走 web_fetch 直接抓取已知 URL | 「已尝试 2 轮搜索仍未获取足够素材，以下维度标记为 [外部来源待补充]，建议你提供手头已有的链接/文档」 |
| Gate 门禁连续不通过 | 同一 Gate 失败 3 次后自动降级通过，标注「已知限制」 | 「XX Gate 门禁连续 3 次未通过，已降级处理继续推进。当前版本的已知限制：[具体内容]」 |
| 流程中断后恢复 | 从 `temp/tiangong-checkpoint-{stage}.md` 读取最近 checkpoint，从中断点继续 | 「检测到 XX 角色的设计进度已保存至蒸馏{N}，是否从该阶段继续？」 |
